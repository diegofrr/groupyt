...
const Modal: React.FC = () => {
  ...
  useEffect(() => {
    if (titleInput.trim() === "")
      setTitleValid({ valid: false, msg: "Forneça um nome para a sala" });
    else if (titleInput.trim().length < 5)
      setTitleValid({
        valid: false,
        msg: "O nome da sala precisa ter, pelo menos, 5 caracteres.",
      });
  ...
  const usernameIsvalid = useCallback(() => {
    let isValid = username !== "" && username !== null && username.length > 2;
    ...
    <ModalContainer isOpen={modalIsOpen}>
      <ModalContent>
        <ModalHeader>
          ...
          <ModalTitleContainer>
            <ModalTitle editing={editing}>{roomTitle}</ModalTitle>
            {editing && (
              <Form onSubmit={handleSaveTitle}>
                <ModalTitleInput
                  maxLength={20}
                  valid={titleValid.valid}
                  value={titleInput}
          ...
          <>
            <NameContainer>
              ...
              <UsernameInput
                ...
                autoComplete="off"
              />
              <LetterCounter letters={username.length}>
                {username.length}/15
              </LetterCounter>
            </NameContainer>
            ...
            <AvatarsContainer>
              <AvatarsHeader>
                <ModalLabel>Escolha um avatar</ModalLabel>
                <AvatarGenreContainer>
                  <GenreOption
              ...
              <AvatarsContent>
                <AvatarsScroll>
                  {genre === "male"
                    ? maleAvatars.map((item) => (
                        <Avatar
                          selected={item === avatar}
                          key={item}
                          onClick={() => handleSelectedAvatar(item)}
                        >
                          <Image
                ...
                <AvatarsCredits>Designed by Kubanek</AvatarsCredits>
              </AvatarsContent>
            </AvatarsContainer>
